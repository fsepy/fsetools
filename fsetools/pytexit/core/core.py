...
class LatexVisitor(ast.NodeVisitor):
    ...
    def convert_symbols(self, expr):
        ...
        elif m in ['lbd', 'lambda_']:  # lambda is not a valid identifier in Python so people use other things
            m = r'\lambda'

        elif m in ['Lbd', 'Lambda']:
            m = r'\Lambda'
    ...
    def visit_Compare(self, n):
        ...
        return r'%s%s' % (self.visit(n.left), ''.join(['%s%s' % (visit_Op(n.ops[i]),
                                                                 self.visit(n.comparators[i])) for i in
                                                       range(len(n.comparators))]))
