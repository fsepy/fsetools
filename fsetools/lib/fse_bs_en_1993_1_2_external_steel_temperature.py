...
class ExternalSteelTemperatureFullyEngulfedColumn(ReportBaseClass):
    def __init__(
            self,
            **kwargs
    ):
    ...
    def make_latex_sections(self, section_title: str = None) -> list:
        ...
        if section_title:
            ...
            sections.append(Section(title='Heat transfer to external steel work'))
    ...
    def section_2_inputs(input_kwargs: dict):
        ...
        symbols = [
            'C_1', 'C_2', 'C_3', 'C_4', 'lambda_1', 'lambda_3', 'sigma', 'T_z', 'T_o', 'd_1', 'd_2', 'w_t', 'L_H',
            'L_L', 'h_eq', 'T_f', 'alpha', 'w_f',
        ]
    ...
    def __calculation(**input_kwargs):
        ...
        if 'phi_f' not in input_kwargs:
            ...
            if not all([i in input_kwargs for i in ['phi_z_1', 'phi_z_2', 'phi_z_3', 'phi_z_4']]):
                input_kwargs.update(clause_b_1_4_1_phi_f_i(**input_kwargs))
                _latex_equation_header.append(
