...
from ..etc.latex import make_alginat_equations, make_summary_table
from ..libstd.bs_en_1991_1_2_2002_annex_b import clause_b_4_1_10_T_z, clause_b_4_2_9_T_z, clause_b_4_1_12_alpha_c, \
    clause_b_4_2_11_alpha_c
from ..libstd.bs_en_1993_1_2_2005_annex_b import *
    ...
    def __calculation(**input_kwargs):
        ...
        def helper_L_x(
            ...
            if is_forced_draught is False:
                if d_aw + 0.5 * d_2 > 0:
                    ...
                    L_x = sqrt(
                        ((2. / 3. * h_eq) / (2 * L_H) * (lambda_4 + 0.5 * d_1)) ** 2 + (lambda_4 + 0.5 * d_1) ** 2) + (
                                      d_aw + 0.5 * d_2)
                else:
        ...
        def helper_L_x_1(
            ...
            if is_forced_draught is True:
                ...
                if d_aw > 0.:
                    # if the location L_x_1 is engulfed in flame
                    L_x_1 = sqrt(((2. / 3. * h_eq) / (2 * L_H) * (lambda_4 + 0.5 * d_1)) ** 2 + (
                                lambda_4 + 0.5 * d_1) ** 2) + d_aw
                elif d_aw <= 0:
                    # if the location L_x_1 is outside of the flame
                    L_x_1 = sqrt(
                        ((2 / 3 * h_eq) / (2 * L_H) * (lambda_4 + 0.5 * d_1)) ** 2 + (lambda_4 + 0.5 * d_1) ** 2)
        ...
        def helper_L_x_2(
            ...
            if is_forced_draught is True:
                L_x_2 = helper_L_x_1(h_eq=h_eq, L_L=L_L, L_H=L_H, d_1=d_1, d_aw=d_aw, lambda_4=lambda_4,
                                     is_forced_draught=is_forced_draught)['L_x_1']
            elif is_forced_draught is False:
                L_x_2 = helper_L_x_1(h_eq=h_eq, L_L=L_L, L_H=L_H, d_1=d_1, d_aw=d_aw, lambda_4=lambda_4,
                                     is_forced_draught=is_forced_draught)['L_x_1'] + d_2
            else:
                raise ValueError
        ...
        if not all([i in input_kwargs for i in ['phi_z_1', 'phi_z_2', 'phi_z_3', 'phi_z_4']]):
            ...
            _latex_equation_header.append(
                'Clause B.1.4 (1), the radiative heat flux from an opening for each of the faces 1, 2, 3 and 4 of the beam is:')
            _latex_equation_content.append(input_kwargs['_latex'])
        ...
        if not all([i in input_kwargs for i in ['I_z_1', 'I_z_2', 'I_z_3', 'I_z_4']]):
            ...
            if is_forced_draught is False:  # for no forced draught conditions
                ...
                if is_flame_above_steel is True and is_immediately_next_to_facade is False:  # for beam fully engulfed and located away from the facade, i.e. `lambda_4` > 0
                    ...
                    _latex_equation_header.append(
                        'Clause B.5.1.2 (3), the emissivity of the flames for each of the faces 1, 2, 3 and 4 of the column are ($C_4$ is set to 0):')
                    _latex_equation_content.append(input_kwargs['_latex'])
                    ...
                    input_kwargs['C_4'] = C_4  # revert C_4 to original input
                    _latex_equation_header.append(
                        'Clause B.5.1.2 (4), the emissivity of the flames for each of the faces 1, 2, 3 and 4 of the column are ($C_4$ is set to 0):')
        ...
        if 'T_z' not in input_kwargs:
            if input_kwargs['is_forced_draught'] is False:
                ...
                _latex_equation_header.append(
                    'BS EN 1991-1-2 Clause B.4.1 (10), the flame temperature along the axis at the centroid of the beam section, $L_{x}$, is:')
            elif input_kwargs['is_forced_draught'] is True:
                # forced draught
                input_kwargs.update(clause_b_4_2_9_T_z(T_w=input_kwargs['T_o'], **input_kwargs))
                _latex_equation_header.append(
                    'BS EN 1991-1-2 Clause B.4.2 (9), the flame temperature along the axis at the centroid of the beam section, $L_{x}$, is:')
            else:
