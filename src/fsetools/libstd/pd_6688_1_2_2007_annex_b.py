...
class EquivalentTimeOfFireExposure:
    ...
    def b_2_ventilation_factor(A_vv: float, A_vh: float, A_f: float, H: float) -> float:
        ...
        if A_f < 100. and A_vh == 0.:
            raise ValueError(
                f'Equation B.2 is applicable for large compartment or small compartment with horizontal openings in the roof')
    ...
    def risk_factor_associated_with_height(self, occupancy: int, height: float):
        ...
        if occupancy == self.OCCUPANCY_RESIDENTIAL_DWELLING:
            ...
            raise ValueError(
                f'Unidentified occupancy type, {occupancy}, check class property beginning with OCCUPANCY_ for available occupancy types')
